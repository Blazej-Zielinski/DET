...
class StrategiesEnum(Enum):
    ...
    RAND_TO_BEST_1_CODE_MOD = "rand_to_best_1_code_mod"
...
def mutation_current_to_rand_1(base_member: Member, members: list[Member], f: float) -> Member:
    """
                Formula: v_ij = x_base + rand(x_r1 - x_base) + F(x_r2 - x_r3)
    """
    r = random.uniform(0, 1)
    new_member = copy.deepcopy(base_member)
    new_member.chromosomes = (base_member.chromosomes + (members[0].chromosomes - base_member.chromosomes) * r + (
            members[1].chromosomes - members[2].chromosomes) * f)

    return new_member


def mutation_rand_to_best_1_code_modified(best_member: Member, members: list[Member], f: float) -> Member:
    """
            Formula: v_ij = x_r1 + F(x_best - x_r2) + F(x_r3 - x_r4)
    """
    new_member = copy.deepcopy(members[0])
    new_member.chromosomes = (members[0].chromosomes + (best_member.chromosomes - members[1].chromosomes) * f + (
            members[2].chromosomes - members[3].chromosomes) * f)
